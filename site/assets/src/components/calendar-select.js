...
export class CalendarSelect extends LitElement {
    ...
    nextView(month) {
        this.shadowRoot
            ...
            .forEach((element) => element.classList.remove('selected-time'))
    ...
    selectDay(event, date) {
        ...
        if (days.length === 0) {
            this.dispatchEvent(
                new CustomEvent('day-added', { detail: { date } }),
            )
        } else {
            days.forEach(({ id }) => {
                this.dispatchEvent(
                    new CustomEvent('day-removed', { detail: { id } }),
                )
            })
        }
        this.shadowRoot
    ...
    getDaysOfTheWeekInitials(localeName = 'en-US', weekday = 'long') {
        ...
        return [...Array(7).keys()].map((day) =>
            format(
                new Date().getTime() - (now.getDay() - day) * dayInMilliseconds,
            ),
        )
    }
    ...
    buildCalendarDays(localeName = 'en-US', monthDate) {
        ...
        const monthDays = []
        const format = new Intl.DateTimeFormat(localeName, { day: 'numeric' })
            .format
    ...
    addMonth() {
        const newEndDate = DateTime.fromISO(this.endDate)
            .plus({ months: 1 })
            .endOf('month')
            .toISODate()
        this.dispatchEvent(
            new CustomEvent('calendar-extended', { detail: { newEndDate } }),
        )
    ...
    renderCalendar(monthDate) {
        const weekDayNames = this.getDaysOfTheWeekInitials(
            navigator.language,
            'narrow',
        )
        ...
        return html`
            ${weekDayNames.map(
                (name) => html` <div class="cell week-day">${name}</div> `,
            )}
            ${map(
                range(dayOfWeekNumber % 7),
                (i) => html` <div class="cell"></div> `,
            )}
            ${monthDays.map(
                (day) => html`
                    <div
                        class="cell day ${day.disabled
                            ? 'disabled'
                            : ''} ${this.isSelected(day.key)
                            ? 'selected-day'
                            : ''} ${this.showToday && day.key === this.today
                            ? 'today'
                            : ''}"
                        data-day=${day.key}
                        @click=${(event) =>
                            !day.disabled && this.selectDay(event, day.key)}
                    >
                        ${day.formatted}
                    </div>
                `,
    ...
    renderSlider() {
        ...
        const monthDate =
            this.monthToShow ||
            DateTime.max(now, DateTime.fromISO(this.startDate))
        ...
        return html`
            <div class="calendar-wrapper">
                <div class="calendar">
                    ...
                    <h3 class="month-title">${monthDate.toFormat('LLLL y')}</h3>
                    <button
                        class="month-next"
