...
const courseViews = ['slideshow', 'guide']
...
export class CoursePresenter extends LitElement {
    ...
    firstUpdated() {
        ...
        languageSelectors.forEach(function (languageSelector) {
            languageSelector.addEventListener('click', () => {
                ...
                if (
                    urlPieces.length > 0 &&
                    jsObject.zume_languages.includes(urlPieces[0])
                ) {
    ...
    handleHistoryPopState() {
        const url = new URL(location.href)
        const sessionIndex = url.searchParams.has('session')
        ...
        document
            .querySelector('.js-off-canvas-overlay')
    ...
    render() {
        ...
        const containerClass =
            this.type === 'intensive' ? 'container-xsm' : 'container-sm'
        ...
        return html`
                      ...
                      <div class="course-index | bg-brand-gradient">
                          ...
                          <div class="${containerClass}" data-max-width="750">
                              <div class="grid | grid-min-8rem gutter0">
                                  ${this.zumeSessions.map(
                                      (session, sessionNumber) => html`
                                          <button
            ...
            <nav
                class="bg-white px-0 text-center | presenter-menu off-canvas ${this
                    .dir === 'rtl'
                    ? 'position-right'
                    : 'position-left'} justify-content-between py-1"
                id="offCanvas"
                data-off-canvas
            ...
            >
                ...
                <div class="stack">
                    <div class="stack">
                        ...
                        >
                            ${Object.values(this.menu).map(
                                ({ title, submenu }, sessionNumber) => html`
                                    <li>
                                        <a
                                            class="session-link"
                                            data-session-number="${sessionNumber}"
                                        ...
                                        <ul
                                            class="menu vertical nested ${this
                                                .lessonIndex === sessionNumber
                                                ? 'is-active'
                                                : ''}"
                                        >
                                            ${submenu.map(
